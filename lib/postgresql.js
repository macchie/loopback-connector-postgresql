...
PostgreSQL.prototype.buildSelect = function(model, filter) {
  ...
  if (filter) {
    if (filter.where) {
      const whereStmt = this.buildWhere(model, filter.where, filter.fuzzySearch);
...
PostgreSQL.prototype.buildExpression = function(columnName, operator,
  operatorValue, propertyDefinition, fuzzy) {
    console.log(operator, operatorValue, fuzzy)
  switch (operator) {
    case 'like':
 ...
 */
PostgreSQL.prototype.buildWhere = function(model, where, fuzzy) {
  const whereClause = this._buildWhere(model, where, fuzzy);
  if (whereClause.sql) {
 ...
 */
PostgreSQL.prototype._buildWhere = function(model, where, fuzzy) {
  ...
  for (const key in where) {
    ...
    if (key === 'and' || key === 'or') {
      ...
      if (Array.isArray(clauses)) {
        for (let i = 0, n = clauses.length; i < n; i++) {
          const stmtForClause = self._buildWhere(model, clauses[i], fuzzy);
    ...
    if (expression === null || expression === undefined) {
      ...
      sqlExp = self.buildExpression(columnName, operator, columnValue, p, fuzzy);
      stmt.merge(sqlExp);
    } else {
